        		...
        		if($statement->rowCount() > 0)
        		{
        			foreach($statement->fetchAll() as $row)
        			{
        				...
        				<tr class="tr">
        					...
        					<td class="td">
        						<a style="background-color:green;text-decoration:none;color:white;" href="updatebook.php?an='.$row['acc_no'].'&cn='.$row['call_no'].'&bn='.$row['book_name'].'&ba='.$row['book_author'].'&pub='.$row['publisher'].'&nc='.$row['book_no_of_copy'].
								'&y='.$row['year'].'&isbn='.$row['book_isbn_number'].'&price='.$row['book_price'].'&page='.$row['page'].'">Edit</a>
                                <a style="background-color:red;text-decoration:none;color:white;" href="deletebook.php?cn='.$row['call_no'].'">delete</a>
        					</td>
        				</tr>
        				';
        			}
